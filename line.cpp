 ...
 */
public
int is_ascii_char(LWCHAR ch) { return (ch <= 0x7F); }
...
void plinenum(POSITION pos) {
  ...
  if (status_col) {
    ...
    if (c != 0)
      ...
      c = ' ';
      if (start_attnpos != NULL_POSITION && pos >= start_attnpos &&
          pos <= end_attnpos)
        a |= AT_HILITE;
    }
 ...
 */
public
void pshift_all(VOID_PARAM) { pshift(column); }
...
static int store_prchar(LWCHAR c, POSITION pos) {
   ...
   */
  if (column + (int)strlen(s) - 1 + pwidth(' ', binattr, 0) +
          attr_ewidth(binattr) >
      sc_width)
    return 1;
...
static int do_append(LWCHAR ch, char *rep, POSITION pos) {
  ...
  if (ch == '\b') {
     ...
     */
    if (curr <= lmargin || column <= lmargin ||
        (attr[curr - 1] & (AT_ANSI | AT_BINARY)))
      STORE_PRCHAR('\b', pos);
 ...
 */
public
void set_status_col(int c) { set_linebuf(0, c, AT_NORMAL | AT_HILITE); }
...
POSITION back_raw_line(POSITION curr_pos, char **linep, int *line_lenp) {
  ...
  for (;;) {
    ...
    if (n <= 0) {
       ...
       */
      for (fm = linebuf + old_size_linebuf - 1, to = linebuf + size_linebuf - 1;
           fm >= linebuf; fm--, to--)
        *to = *fm;
