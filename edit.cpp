...
void init_textlist(struct textlist* tlist, char* str)
{
  ...
  for (s = str; s < tlist->endstring; s++) {
    if (meta_quoted) {
      ...
      if (*s == less::Globals::closequote)
        delim_quoted = 0;
    } else /* (!delim_quoted) */
    {
      if (*s == less::Globals::openquote)
        delim_quoted = 1;
      else if (*s == ' ')
        *s = '\0';
    }
...
int edit_ifile(ifile::Ifile* requestedIfile)
{
  ...
  if (!(chflags & CH_HELPFILE)) {
    if (less::Globals::namelogfile != nullptr && is_tty)
      use_logfile(less::Globals::namelogfile);
...
void use_logfile(char* filename)
{
   ...
   */
  if (!exists || less::Globals::force_logfile) {
    /*
...
loop:
  ...
  case 'o':
     ...
     */
    less::Globals::logfile = creat(filename, 0644);
    break;
  case 'A':
  case 'a':
    /*
     ...
     */
    less::Globals::logfile = open(filename, OPEN_APPEND);
    if (lseek(less::Globals::logfile, (off_t)0, SEEK_END) == BAD_LSEEK) {
