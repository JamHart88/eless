...
/** Final trace close */
static void __NON_INSTRUMENT_FUNCTION__ gnu_ptrace_close(void)
{
    FILE *trace;
...
static int __NON_INSTRUMENT_FUNCTION__ gnu_ptrace_init(void)
{
...
void __NON_INSTRUMENT_FUNCTION__ gnu_ptrace(char *what, void *p)
{
    ...
    if (first)
    {
        ...
        fprintf(trace, "%s %s %p\n", REFERENCE_OFFSET, GET(STR, PTRACE_REFERENCE_FUNCTION),
                (void *)GET(DEF, PTRACE_REFERENCE_FUNCTION));
#endif
        first = 0;
    };
    fprintf(trace, "%s %p\n", what, p);
    fprintf(trace, "%s: func = %p/n", __FUNCTION__, p);
    fclose(trace);
    return;
}
...
{
    gnu_ptrace((char *)FUNCTION_ENTRY, this_fn);
    (void)call_site;
}
...
{
    gnu_ptrace((char *)FUNCTION_EXIT, this_fn);
    (void)call_site;
}
*/ 
...
void __cyg_profile_func_enter (void *this_fn, void *call_site)
{
printf("Function Entry : %p %p %s \n", this_fn, call_site, __FUNCTION__);
}

void __cyg_profile_func_exit (void *this_fn, void *call_site)
{
printf("Function Exit : %p %p %s \n", this_fn, call_site, __FUNCTION__);
}
