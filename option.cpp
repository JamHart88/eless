...
static char* opt_desc(struct loption* o)
{
  ...
  if (o->oletter == OLETTER_NONE)
    ignore_result(snprintf(buf, sizeof(buf), "--%s", o->onames->oname));
  else
    ignore_result(snprintf(buf, sizeof(buf), "-%c (--%s)", o->oletter, o->onames->oname));
  return (buf);
}
...
char* propt(int c)
{
  ...
  ignore_result(sprintf(buf, "-%s", charset::prchar(c)));
  return (buf);
}
...
void scan_option(char* s)
{
  ...
  while (*s != '\0') {
    ...
    if (optname == NULL) {
      printopt = propt(optc);
      lc       = islower(optc);
      o        = opttbl::findopt(optc);
    } else {
      printopt = optname;
      lc       = islower(optname[0]);
      o        = opttbl::findopt_name(&optname, NULL, &err);
      s        = optname;
      optname  = NULL;
      if (*s == '\0' || *s == ' ') {
        /*
    ...
    char* validchars)
{
  ...
  *p_str = (char*)utils::ecalloc(strlen(s) + 1, sizeof(char));
  ...
  for (p = s; *p != '\0'; p++) {
    if (Option::opt_use_backslash && *p == '\\' && p[1] != '\0') {
      /* Take next char literally. */
      ++p;
    } else {
      if (*p == END_OPTION_STRING || (validchars != NULL && strchr(validchars, *p) == NULL))
...
{
  if (!option::Option::less_is_more)
    return option::Option::quit_at_eof;
  /* When less_is_more is set, the -e flag semantics are different. */
  return option::Option::quit_at_eof ? option::OPT_ONPLUS : option::OPT_ON;
}
