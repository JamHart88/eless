 ...
 * search for const-time lookup if the next search is near enough to
...
static int set_pattern(struct pattern_info* info, char* pattern, int search_type)
{
  ...
  if (is_ucase_pattern && caseless != option::OPT_ONPLUS)
    is_caseless = 0;
  else
...
int is_hilited(position_t pos, position_t epos, int nohide, int* p_matches)
{
  ...
  if (!status_col && start_attnpos != NULL_POSITION && pos < end_attnpos && (epos == NULL_POSITION || epos > start_attnpos))
    /*
...
static void add_hilite(struct hilite_tree* anchor, struct hilite* hl)
{
  ...
  for (;;) {
    ...
    /*
     * case 3 - parent and uncle are red, jump::repaint them black,
...
void chg_hilite(void)
{
  ...
  if (hilite_search == option::OPT_ONPLUS)
    /*
     * Display highlights.
     */
    hilite_screen();
  ...
  if (position::empty_screen()) {
    ...
    if (how_search == option::OPT_ON) {
      ...
      if (search_type & SRCH_FORW)
        ...
        sindex = 0; /* TOP */
    } else if (how_search == option::OPT_ONPLUS && !(search_type & SRCH_AFTER_TARGET)) {
      /*
...
static int search_range(position_t pos, position_t endpos, int search_type, int matches, int maxlines, position_t* plinepos, position_t* pendpos)
{
  ...
  for (;;) {
    ...
    if (search_type & SRCH_FORW) {
       ...
       */
      linepos = pos;
      pos     = line::forw_raw_line(pos, &line, &line_len);
      if (linenum != 0)
        linenum++;
    } else {
      /*
       ...
       * starting position of that line in linepos.
       */
      pos     = line::back_raw_line(pos, &line, &line_len);
      linepos = pos;
      if (linenum != 0)
        linenum--;
    }
    ...
    if (prev_pattern(&search_info)) {
      ...
      if (line_match) {
        ...
        if (search_type & SRCH_FIND_ALL) {
           ...
           */
#if HILITE_SEARCH
          if (hilite_search == option::OPT_ON) {
            /*
...
#if HILITE_SEARCH
  if (hilite_search == option::OPT_ONPLUS && !hide_hilite)
    hilite_screen();
#endif
...
int search(int search_type, char* pattern, int n)
{
  ...
  if (pattern == NULL || *pattern == '\0') {
    ...
    if (hilite_search == option::OPT_ONPLUS || status_col) {
      /*
       * Highlight any matches currently on screen,
       * before we actually start the search.
       */
  ...
  if (n != 0) {
     ...
     */
#if HILITE_SEARCH
    if ((hilite_search == option::OPT_ON || status_col) && n > 0)
      /*
...
#define SEARCH_MORE (3 * size_linebuf)
   ...
   */
  spos = line::back_raw_line(spos + 1, (char**)NULL, (int*)NULL);
  ...
  if (maxlines < 0)
    ...
    for (i = 0; i < maxlines; i++)
      max_epos = line::forw_raw_line(max_epos, (char**)NULL, (int*)NULL);
  }
  ...
  if (prep_startpos == NULL_POSITION || (epos != NULL_POSITION && epos < prep_startpos) || spos > prep_endpos) {
    /*
    ...
    for (;;) {
      ...
      if (prep_endpos == NULL_POSITION || nprep_endpos > prep_endpos) {
        if (new_epos >= nprep_endpos && is_filtered(new_epos - 1)) {
          spos = nprep_endpos;
          epos = line::forw_raw_line(nprep_endpos, (char**)NULL, (int*)NULL);
          if (epos == NULL_POSITION)
            break;
          maxlines = 1;
          continue;
      ...
      if (prep_startpos == NULL_POSITION || nprep_startpos < prep_startpos) {
        if (nprep_startpos > 0 && is_filtered(nprep_startpos)) {
          epos = nprep_startpos;
          spos = line::back_raw_line(nprep_startpos, (char**)NULL, (int*)NULL);
